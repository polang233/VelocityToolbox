...
修改后执行 `/vtoolbox reload` 或 `/velocity reload`，用 `/vtoolbox info` 查看模块开关和子服规则数量。重载影响之后的连接请求，不踢出已经进入子服的玩家。将 `server-versions.enabled` 改为 `false` 后重载即可关闭模块。模块与资源包托管分别解析，一方重载失败不阻止另一方尝试重载。

## 拒绝行为与配置错误

切服被拒时，玩家留在当前子服，并收到版本要求与自身协议号。首次连接或当前已无子服连接时，会断开并显示同样的原因；不会自动选择另一个入口服。入口服应允许你希望接待的客户端版本。
...
Minecraft 多个补丁版本可能共用一个协议。例如 1.20 与 1.20.1 都是 763，配置任何一个都会同时匹配两者；1.18 与 1.18.1 也一样。模块的提示会显示该协议对应的版本组。仅凭握手协议无法进一步区分这些版本。
